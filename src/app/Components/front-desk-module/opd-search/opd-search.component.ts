    ...
    debugger;                             
       ...
       public EditBilling(index: any, SelectedPatient: any )
       {
        debugger 
         ...
         localStorage.setItem('encId',SelectedPatient.encounterId)
         this.himsservice.GetBillingDetailsByEncounterId(SelectedPatient.encounterId).subscribe((result) => {
          debugger
          if(result.length>0){
            this.rout.navigateByUrl('/FrontDesk/opdbill-Search');
          }
          else{
          this.rout.navigateByUrl('/FrontDesk/opdbilling');
          }
         })
         //alert(SelectedPatient.encounterId);
