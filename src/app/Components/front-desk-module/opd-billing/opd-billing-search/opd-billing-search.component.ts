...
@Component({
  selector: 'app-opd-billing-search',
  templateUrl: './opd-billing-search.component.html',
  styleUrls: ['./opd-billing-search.component.css',"../../../../../css/buttons.bootstrap4.min.css" ,
...
export class OpdBillingSearchComponent implements OnInit {
  //#region Declarations
  datenow=new Date();
  ...
  AcceptCharactersOnly(event:KeyboardEvent)
  {
   ...
   SearchBillings(Dates:any)
   {
    ...
    if(this.searchform.invalid)
    {
      ...
      this.himsservice.SearchBillings(Dates).subscribe((result)=>
      {
        debugger
        if(result.length!=0)
        {
   ...
   SearchBillingsinload()
   {
    debugger
     ...
     this.himsservice.SearchBillingsbytoday().subscribe((result)=>
    {
      debugger
      if(result.length!=0)
      {
