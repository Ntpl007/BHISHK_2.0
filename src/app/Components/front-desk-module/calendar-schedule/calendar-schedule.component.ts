...
export class CalendarscheduleComponent implements OnInit {
  ...
  Templimittimeslist : any[] = [];
   ...
   selectedTransferProviderid : number =0 
   isscheduleIntravel:any;
   isTranferscheduleIntravel:any;
  ...
  public FillTimes()
    ...
    public FillTimesWithSpecificId(idlestsrtTime:any, idlestendTime:any, scheduleIntravel:any)
    {
      this.Templimittimeslist = [];
      var quarterHours:string[] = [];
      if(parseInt(scheduleIntravel) == 15)
      {
        quarterHours = ["00", "15", "30", "45"];
      }
      else if(parseInt(scheduleIntravel) == 30)
      {
        quarterHours = ["00", "30"];
      }
      else
      ...
      for (var i = idlestsrtTime; i < idlestendTime; i++) {
        for (var j = 0; j < quarterHours.length; j++) {
            var time = i + ":" + quarterHours[j];
            if (i < 10) {
                time = "0" + time;
            }
            this.Templimittimeslist.push(time);
        }
       
      }
      if (idlestendTime < 10) {
        idlestendTime = "0" + idlestendTime;
      }
      this.Templimittimeslist.push(idlestendTime + ":" + "00")
     
    }
    public getTransferDoctors(Id:any)
    {
      ...
      var doctorvalue = (<HTMLSelectElement>document.getElementById('ddlDoctor'));
      this.service.GetDoctorbyspeciality(Id.target.value).subscribe((result)=>{
        //this.doctors=result;
        this.transferdoctors = result;
        
        if(this.selectedTransferProviderid != 0)
        {
          for(var i =0; i<this.transferdoctors.length; i++)
          {
            if(this.transferdoctors[i].providerId == this.selectedTransferProviderid)
            {
              this.transferdoctors.splice(i, 1);
            }
          }
        }
      })
            ...
            (data)=>{
              ...
              for(let i=0; i < result.length; i++){
                ...
                const End = new Date(result[i].endDate); 
                  ...
                  this.isscheduleIntravel = result[i].scheduleInterval;
                  const existingIndex = this.row.findIndex((row) => {
                    const keyValue = row.split('='); 
    ...
    const [time, period] = this.selectedSlot.split(' ');
      ...
      if(!isPast && !isAvailable){
        const dialogRef=this.dialog.open(EditAppointmentsComponent,{
          width:"65%",
          height:"450px",
          data: this.rescheduleappointmetid
      })
    }  
    }
    public onDateSelectTransferdate(event:any)
    {
      ...
      var fTransferDate= (<HTMLInputElement>document.getElementById('fTransferDate')).value;
       ...
       this.limittimeslist = [];
       this.telimittimeslist = [];
     
       this.service.GetResechduleslotsData(this.TransferDate, trandferproviderId.value,this.facility).subscribe((
         (result)=>{
         this.telimittimeslist = result;
       }))
  
       this.service.GetExtraReScheduleDates(this.TransferDate, trandferproviderId.value,this.facility).subscribe((
        (result)=>{
          debugger
        this.transferdates = result;
        for(var i = 0; i < this.transferdates.length; i++)
        {
          var restarttime = this.transferdates[i].startTime;
          var reendtime = this.transferdates[i].endTime;
          this.isTranferscheduleIntravel = this.transferdates[i].scheduleInterval;
          this.FillTimesWithSpecificId(restarttime.hours, reendtime.hours, this.isTranferscheduleIntravel);
          for(var k = 0; k < this.Templimittimeslist.length; k++)
          {
            this.limittimeslist.push(this.Templimittimeslist[k]);
          }
        }
        let outputArray = [];
        outputArray = this.limittimeslist.filter(function (v, i, self) {
          return i == self.indexOf(v);
        });
        outputArray.pop();
        for(var j = 0; j < this.telimittimeslist.length; j++)
          {
            var retime = this.telimittimeslist[j].providerscheduletime.split(":",3); 
            outputArray.splice(outputArray.findIndex(item => item === retime[0] + ":" + retime[1]),1);
          } 
          
        
          const currentTime = new Date();
          let currentdateandtime = currentTime.getFullYear() + "-"+ (currentTime.getMonth() + 1) + "-"+ (currentTime.getDate() < 10 ? "0" + currentTime.getDate() : currentTime.getDate());
          
          if(currentdateandtime == this.TransferDate)
          {
            let completedtimelist: any[] = [];  
          for(var kl = 0; kl < outputArray.length; kl ++)
          {
            var regex = new RegExp(':', 'g'),
            timeStr1 = outputArray[kl],
            timeStr2 = currentTime.getHours() + ":"+currentTime.getMinutes();
            var ttt = currentTime.getMinutes();
            if(currentTime.getMinutes() < 10)
            {
             timeStr2 =  currentTime.getHours() + ":"+"0"+currentTime.getMinutes();
            }
            if(parseInt(timeStr1.replace(regex, ''), 10) < parseInt(timeStr2.replace(regex, ''), 10)){
              completedtimelist.push(outputArray[kl]);
            }
          }
          outputArray = outputArray.filter(item => completedtimelist.indexOf(item) < 0);
  
          }
        this.limittimeslist = [];
        for(var l = 0; l < outputArray.length; l++)
        {
          this.limittimeslist.push(outputArray[l]);
        }
        if(this.limittimeslist.length == 0)
        {
          this.FillTimes();
        }
      }))
    ...
    this.service.GetResechduleslotsData(this.RescheduleDate, this.providerId,this.facility).subscribe((
      ...
      this.service.GetExtraReScheduleDates(this.RescheduleDate, this.providerId,this.facility).subscribe((
        (result)=>{
        this.rescheduledates = result;
        for(var i = 0; i < this.rescheduledates.length; i++)
        {
          var restarttime = this.rescheduledates[i].startTime;
          var reendtime = this.rescheduledates[i].endTime;
          this.FillTimesWithSpecificId(restarttime.hours, reendtime.hours, this.isscheduleIntravel);
          for(var k = 0; k < this.Templimittimeslist.length; k++)
          {
            this.limittimeslist.push(this.Templimittimeslist[k]);
          }
        ...
        if(currentdateandtime == this.RescheduleDate )
      {
        let completedtimelist: any[] = [];  
        for(var kl = 0; kl < outputArray.length; kl ++)
        {
     ...
     public onTransferStarttimeChanged()
     {
      ...
      var minutevalue = parseInt(transferstarttime.value.split(':', 2)[1]) + parseInt(this.isTranferscheduleIntravel);
      var hourtime = 0;
      if(this.isTranferscheduleIntravel == "15")
      {
        if(transferstarttime.value.split(':', 2)[1] != "45")
        {
          transferendtime.value = transferstarttime.value.split(':', 2)[0] + ":" + minutevalue;
        }
        else
        {
            hourtime = Number(transferstarttime.value.split(':', 2)[0]) + 1;
            transferendtime.value =  hourtime + ":" + "00"; 
        }
      }
      else if(this.isTranferscheduleIntravel == "30")
      {
        if(transferstarttime.value.split(':', 2)[1] != "30")
        {
          transferendtime.value = transferstarttime.value.split(':', 2)[0] + ":" + minutevalue;
        }
        else
     ...
     public onStarttimeChanged()
     {
      ...
      var minutevalue = parseInt(reschedulestarttime.value.split(':', 2)[1]) + parseInt(this.isscheduleIntravel);
      var hourtime = 0;
      if(this.isscheduleIntravel == "15")
      {
        if(reschedulestarttime.value.split(':', 2)[1] != "45")
        {
          rescheduleendtime.value = reschedulestarttime.value.split(':', 2)[0] + ":" + minutevalue;
        }
        else
        {
            hourtime = Number(reschedulestarttime.value.split(':', 2)[0]) + 1;
            rescheduleendtime.value =  hourtime + ":" + "00"; 
        }
      }
      else if(this.isscheduleIntravel == "30")
      {
        if(reschedulestarttime.value.split(':', 2)[1] != "30")
        {
          rescheduleendtime.value = reschedulestarttime.value.split(':', 2)[0] + ":" + minutevalue;
        }
        else
     ...
     transferPatient()
     {
      ...
      var doctoridvalue = (<HTMLSelectElement>document.getElementById('ddlTransferDoctor')).value;
      appointment.StartTime = rescheduledatevalue + " " + reschedulestarttime;
      appointment.EndTime = rescheduledatevalue + " " + rescheduleendtime;
      appointment.DoctorId = parseInt(doctoridvalue);
  ...
  Resetmodel()
  {
    ...
    transferSlot() {
      ...
      var apointmentdate = (<HTMLSelectElement>document.getElementById('txtTransferAppointmentDate'));
      apointmentdate.value = this.AppdateModel + " " + this.selectedSlot;
      
      var ddldoctorname = (<HTMLSelectElement>document.getElementById('ddlDoctor'));
      var doctorname = (<HTMLSelectElement>document.getElementById('txtTransferDoctorName'));
      //doctorname.value = ddldoctorname.value;
      this.selectedTransferProviderid = parseInt(ddldoctorname.value);
      for(var i =0; i<this.doctors.length; i++)
      {
        if(this.doctors[i].providerId == ddldoctorname.value)
        {
    ...
    var endTIme = new Date();
    endTIme.setHours(hours);
    endTIme.setMinutes(minutes + parseInt(this.isscheduleIntravel));
