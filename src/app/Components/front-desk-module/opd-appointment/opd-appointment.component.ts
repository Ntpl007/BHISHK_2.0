...
@Component({
  ...
  styleUrls: ['./opd-appointment.component.css',"../../../../css/buttons.bootstrap4.min.css" ,
...
export class OpdAppointmentComponent implements OnInit {
  form!: FormGroup;
    submitted = false;
;
...
getDoctorId(id:any){
  this.service.GetDoctorbyspeciality(id).subscribe((result : Doctors[])=>(this.docs=result));
  this.service.GetRefDoctorbyspeciality(id).subscribe((result : RefDoctor[])=>(this.refd=result));
}
   GetDoctor(id:any)
   {
...
SaveOp(data :any)
    ...
    if((data.City==undefined&&data.Village==undefined)||(data.City==""&&data.Village==""))
  {
   ...
   this.MyInputData.CreatedBy=this.userservice.getUserName()
    ...
    this.MyInputData.State=this.patientDetails[0].statE_ID;
    this.MyInputData.District=this.patientDetails[0].districT_ID;
    this.MyInputData.Prefix=this.patientDetails[0].prefix;

    this.MyInputData.ReligionId=this.patientDetails[0].religioN_ID;
    this.MyInputData.NationalityId=this.patientDetails[0].nationalitY_ID;
    this.MyInputData.SpacialityId=this.patientDetails[0].specialitY_ID;
    this.MyInputData.DoctorId=this.patientDetails[0].doctoR_ID;
    this.MyInputData.PatientId=this.patientDetails[0].patienT_ID;
   this.MyInputData.AppointmentId=this.patientDetails[0].appointmentId;
...
//#endregion 
debugger
if(this.MyInputData.PatientId!=0){
  this.controlpath='CreateOP/SaveOPExisting'
}else{
 ...
 }else
 {
  ...
  this.http.post<any>(this.service.Serverbaseurl2+ this.controlpath,this.MyInputData).subscribe(
    (result)=>{
      debugger;
      var msg=result;
      let patientTempId=localStorage.getItem('patientTempId')
      this.service.UpdatePatientIdInPatientTemp(patientTempId,result.patientID).subscribe((result)=>{
debugger
let r=result
      });
...
onKeyDownForPincode(event: KeyboardEvent) {
      ...
      if(txt.length==0)
  {
    ...
    bindPatientAppointmentDetails(patientDetails:any){
     
      patientDetails[0].datE_OF_BIRTH=this.datePipe.transform(patientDetails[0].datE_OF_BIRTH,'dd-MM-yyyy')
      this.opform.get('FirstName').patchValue(patientDetails[0].firsT_NAME);
      this.opform.get('LastName').patchValue(patientDetails[0].lasT_NAME);
      this.opform.get('dob').patchValue(patientDetails[0].datE_OF_BIRTH);
      
      this.opform.get('Sex').patchValue(patientDetails[0].sex);
      this.opform.get('Aadhaar').patchValue(patientDetails[0].aadhaR_NO);
      this.opform.get('MobileNumber').patchValue(patientDetails[0].mobilE_NUMBER);
      this.opform.get('ReligionId').patchValue(patientDetails[0].religioN_ID);
      this.opform.get('NationalityId').patchValue(patientDetails[0].nationalitY_ID);
      this.opform.get('HouseNo').patchValue(patientDetails[0].dooR_NO);
      this.opform.get('State').patchValue(patientDetails[0].statE_ID);
      this.opform.get('District').patchValue(patientDetails[0].districT_ID);
      this.opform.get('City').patchValue(patientDetails[0].city);
      this.opform.get('Village').patchValue(patientDetails[0].village);
      this.opform.get('Pincode').patchValue(patientDetails[0].pincode);
      this.opform.get('DoctorId').patchValue(patientDetails[0].doctoR_ID);
      this.opform.get('SpacialityId').patchValue(patientDetails[0].specialitY_ID);
      this.opform.get('Prefix').patchValue(patientDetails[0].prefix);
      this.opform.get('EmailId').patchValue(patientDetails[0].email);
      this.dobModel=patientDetails[0].datE_OF_BIRTH;
      // (<HTMLInputElement>document.getElementById('dob')).value=actual
      this.getAgeInDays(this.dateservice.GlobalStringDateFormat(patientDetails[0].datE_OF_BIRTH) );
      this.GetDistricts(patientDetails[0].statE_ID);
      this.getDoctorId(patientDetails[0].specialitY_ID);
      let OrganizationId=localStorage.getItem('organizationId')
      let FacilityId=localStorage.getItem('facilityId')
      this.service.GetConsultationAmount(this.patientDetails[0].doctoR_ID,OrganizationId,FacilityId,725).subscribe((result : any)=>{
         // alert(result);
  ...
  ngOnInit(): void {
    ...
    let patientTempId=localStorage.getItem('patientTempId')
    //alert(patientTempId);
    if(patientTempId!=null){
      this.service.GetPatientDetailsByAppointmentId(patientTempId).subscribe((result : any)=>{
        this.patientDetails=result;
        debugger
      this.bindPatientAppointmentDetails(result);
      });
...
this.opform=this.formbuilder.group({
  ...
  Aadhaar:[''],
  MobileNumber:[''],
  ReligionId:[{value:'0',disabled: true}],
  NationalityId:[{value:'0',disabled: true}],
  HouseNo:[''],
  ...
  Village:[''],
  Pincode:[''],
  SpacialityId:[{value:'0',disabled: true}],
  DoctorId:[{value:'0',disabled: true}],
  RefDoctorId:['0'],
  Prefix:[{value:'0',disabled: true}],
  Occupation:['0',Validators.required],
  CorporateId:['0',Validators.required],
  BenificiaryId:['',Validators.required],
  PaymentAmount:[this.RGA,Validators.required],
